...
class mission:        
    def GET(self, vehicleId):
        try:
            droneAPIUtils.my_logger.info( "#### Method GET of mission ####")
    ...
    def POST(self, vehicleId):
        try:
            droneAPIUtils.my_logger.info( "#### Method POST of mission ####")
            droneAPIUtils.applyHeadders()
            try:
            ...
            droneAPIUtils.my_logger.info( "download existing commands")
            cmds = inVehicle.commands
            cmds.download()
            cmds.wait_ready()
            #clear the commands
            droneAPIUtils.my_logger.info( "clearing existing commands")
            cmds.clear()
            inVehicle.flush()
            missionActionArray = json.loads(web.data())
            droneAPIUtils.my_logger.info( "missionCommandArray:")
            droneAPIUtils.my_logger.info( missionActionArray)
            #add new commands
            for missionAction in missionActionArray:
                droneAPIUtils.my_logger.info(missionAction)
                ...
                cmd = Command(0,0,0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, missionAction['command'],
                    ...
                    lat, lon, altitude)
                droneAPIUtils.my_logger.info( "Add new command with altitude:")
                droneAPIUtils.my_logger.info( altitude)
                cmds.add(cmd)
            inVehicle.flush()
            droneAPIUtils.my_logger.info( "Command added")
            output=getMissionActions(vehicleId) 
        except Exception as e: 
            droneAPIUtils.my_logger.exception(e)
            tracebackStr = traceback.format_exc()
    ...
    def OPTIONS(self, vehicleId):
        try:
            droneAPIUtils.my_logger.info( "#### Method OPTIONS of mission - just here to suppor the CORS Cross-Origin security ####")
            droneAPIUtils.applyHeadders()
...
def getMissionActions(vehicleId) :
    try:
        ...
        cmds.download()
        cmds.wait_ready()
        droneAPIUtils.my_logger.debug( "Mission Commands")
        ...
        for cmd in cmds:
            ...
            droneAPIUtils.my_logger.debug( cmd)
        droneAPIUtils.my_logger.debug( missionlist)
        ...
        outputObj["_links"]={"self":{"href":droneAPIUtils.homeDomain+"/vehicle/"+str(vehicleId)+"/mission","title":"Get the current mission commands from the vehicle."}}
